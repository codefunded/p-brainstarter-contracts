...
async function main() {
  ...
  let tokenAAmount = BigInt((process.env.TOKEN_A_AMOUNT as string) || ethers.parseEther('150000'));
  let tokenBAmount = BigInt((process.env.TOKEN_B_AMOUNT as string) || ethers.parseEther('100000'));
  ...
  const price = encodeSqrtRatioX96(tokenBAmount.toString(), tokenAAmount.toString()).toString();
